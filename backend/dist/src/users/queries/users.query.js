"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UsersQuery = void 0;
const common_1 = require("@nestjs/common");
const drizzle_orm_1 = require("drizzle-orm");
const mysql2_1 = require("drizzle-orm/mysql2");
const database_module_1 = require("../../database/database.module");
const users_schema_1 = require("../../database/schema/users.schema");
let UsersQuery = class UsersQuery {
    db;
    constructor(db) {
        this.db = db;
    }
    async findAll() {
        return await this.db.select().from(users_schema_1.users);
    }
    async findById(id) {
        const result = await this.db
            .select()
            .from(users_schema_1.users)
            .where((0, drizzle_orm_1.eq)(users_schema_1.users.id, id))
            .limit(1);
        return result[0] ?? null;
    }
    async create(dto) {
        const result = await this.db.insert(users_schema_1.users).values({
            email: dto.email,
            name: dto.name,
        });
        return result;
    }
};
exports.UsersQuery = UsersQuery;
exports.UsersQuery = UsersQuery = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, common_1.Inject)(database_module_1.DRIZZLE)),
    __metadata("design:paramtypes", [mysql2_1.MySql2Database])
], UsersQuery);
//# sourceMappingURL=users.query.js.map